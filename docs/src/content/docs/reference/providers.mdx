...
**Availability**: Added in SecretSpec 0.21 and included in default builds; use
...
```bash
doppler://myapp               # the SecretSpec profile names the config
...
**Prerequisites**: An existing Doppler project and config (SecretSpec does not
create them)
...
or by the pinned `CONFIG`. SecretSpec's own project name is unused — the Doppler
...
| Doppler (0.21+) | ✅ Doppler-managed | Cloud (Doppler) | ✅ Yes |
