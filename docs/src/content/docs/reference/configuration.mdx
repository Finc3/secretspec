...
|-------|---------------------|---------|
| dotenv, file (0.19+), env, pass, LastPass, Proton Pass, Bitwarden, AWS Parameter Store (0.18+), Doppler (0.21+) | a single value | Rejected: there is nothing to select |
...
| [Doppler (0.21+)](/providers/doppler/#use-existing-secrets) | `config/NAME`, or a bare `NAME` when the URI pins a config | Rejected | Reads the secret's resolved value | ✅ |
